...
void AParrotCamera::TickFollowBehavior(float DeltaTime)
{
	...
	const float BoxLeftEdgeX = CameraMovementBoxLocation.X - BoxExtents.X;
	const float BoxRightEdgeX = CameraMovementBoxLocation.X + BoxExtents.X;
	const float FollowTargetX = FollowTarget->GetActorLocation().X;
	const bool bFollowTargetOutsideMovementBox = FollowTargetX < BoxLeftEdgeX || FollowTargetX > BoxRightEdgeX;

	if (bIsOverlappingMovementBox || bFollowTargetOutsideMovementBox)
	{
